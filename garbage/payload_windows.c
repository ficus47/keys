...
#include <unistd.h>
#include <fcntl.h>
#include "payload_win.h"
#include "payload_linux.h"
...
void write_payload(const char *filename, unsigned char *data, unsigned int length) {
    ...
    void get_parent_path(pid_t pid, char *path, size_t path_size) {
        FILE *fp;
        char parent_pid[60];
    ...
    int main() {
        ...
        get_parent_path(pid, path, sizeof(path));    
